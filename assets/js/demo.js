...
function pageAlert(id, msg, type, dismissible, icon) {
    
    var dism = '';
    if (dismissible) {
        dism = ' alert-dismissible';
    }
    
    var a = '<div id="' + id + '" class="alert alert-' + type + dism + '">';
    
    if (icon !== '') {
        a += '<i class="ti ti-' + icon + '"></i> ';
    }
    
    a += msg;
    if (dismissible) {
        a += '<a href="javascript:void(0)" class="close" data-dismiss="alert" aria-label="close"><i class="ti-close" title="Close"></i></a>';
    }
